...
//#define TD_SVR "TradingServer"
//#define MK_DATA "MarketData"
...
inline int JgExchage(CString& szCode)
{
	CString szTmp = szCode;
	if (!szTmp.Compare(_T("CZCE")))
	{
		szCode = _CZCE;
	}
	if (!szTmp.Compare(_T("DCE")))
	{
		szCode = _DCE;
	}
	if (!szTmp.Compare(_T("SHFE")))
	{
		szCode = _SHFE;
	}
	if (!szTmp.Compare(_T("CFFEX")))
	{
		szCode = _CFFEX;
	}
...
inline void JgBkName(CString& szIn,BYTE bType)
{
	szIn =_T("");
	switch(bType)
	{
