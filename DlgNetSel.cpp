...
UINT DlgNetSel::TestSvr(LPVOID pParam)
{
	...
	//ASSERT_POINTER(pTest, TESTSVRPARAM);
		...
		for (i = 0; i <iSizeTd; i++)
		{
			for (iSubItem = 0; iSubItem < 2; iSubItem++)
			{
				...
				if (!m_pTdTest)
				{
					...
					//MsgWaitForMultipleObjects(1, &m_pTdTest->m_hThread, FALSE, 250, QS_ALLINPUT); 
				}
...
void DlgNetSel::OnNMClickLstTrade(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (pNMIA)
	{
		...
		//m_szDefTds = m_LstTds.GetItemText(nItem,0);
...
void DlgNetSel::OnNMClickLstMd(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (pNMIA)
	{
		...
		//m_szDefMds = m_LstMds.GetItemText(nItem,0);
		
		//ShowErroTips(m_szDefMds,MY_TIPS);
	}
